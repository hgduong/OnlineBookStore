...
public class BookDetailServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (bookIdStr == null || bookIdStr.isEmpty()) {
            response.sendError(HttpServletResponse.SC_BAD_REQUEST, "Book ID is missing or invalid");
            return;
        }
        ...
        map = reviewService.calAvarageRating(reviews);
...
//        System.out.println("Book from DAO::: " + book);
            ...
            int numOfVote = Math.round( map.get("numOfVote"));
            request.setAttribute("reviews", reviews);
            request.setAttribute("book", book);
            request.setAttribute("rating", Comm.round2decimal((map.get("rating"))));
            request.setAttribute("numOfVote", numOfVote);
